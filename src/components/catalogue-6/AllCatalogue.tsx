 ...
 *
 * FIX #2: buttons now carry `cursor-pointer` explicitly AND a global base rule
 * in index.css restores the pointer hand that Tailwind v4's preflight removed.
 */
...
import CatalogueSkeleton, { ProductGridSkeleton } from "./CatalogueSkeleton";
import {
  joinCatalogue,
  type CatalogueData,
  type CatalogueRow,
...
type Props = {
  ...
  theme?: ThemeMode;
  /** Called when user wants to open the compare view */
  onOpenCompare?: () => void;
};
...
export default function AllCatalogue({
  ...
  // `booting`  → first open: full skeleton while rows are joined/indexed.
  // `filtering`→ any filter/search change: grid skeleton + top progress bar.
  // Without these the heavy join makes taps *feel* hung on real devices.
  const [booting, setBooting] = useState(true);
  const [filtering, setFiltering] = useState(false);

  // First-load skeleton (brief — just long enough to cover the join work).
  useEffect(() => {
    const t = setTimeout(() => setBooting(false), 550);
    return () => clearTimeout(t);
  }, []);

  // Debounce the search box so we don't re-filter on every keystroke.
  useEffect(() => {
    const t = setTimeout(() => setSearch(searchInput), 250);
    return () => clearTimeout(t);
  }, [searchInput]);

  // Any filter change → show a short skeleton so the tap always answers back.
  ...
  useEffect(() => {
    ...
    return () => clearTimeout(t);
  }, [countryCode, brandId, categoryValue, capValue, search]);
  ...
  const selectMetric = useMemo<"HP" | "CFM" | "KW" | "TR" | "MODEL" | "MIXED">(() => {
    ...
    if (kinds.size === 1)
      return Array.from(kinds)[0] as "HP" | "CFM" | "KW" | "TR" | "MODEL";
    return "MIXED";
  }, [preCapRows]);
      ...
      <>
        ...
        >
          ...
          <div className="relative mb-4">
            <span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2" style={{ color: p.textMuted }}>
              <Search size={15} />
            </span>
            <input
              type="search"
              ...
              className="w-full cursor-text rounded-xl py-2.5 pl-9 pr-3 text-sm outline-none transition focus:ring-2"
              style={{
                background: p.inputBg,
                border: `1px solid ${p.cardBorder}`,
                color: p.textPrimary,
                ["--tw-ring-color" as string]: p.accent + "55",
              }}
            />
          </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="block">
              ...
              <select
                ...
                className="w-full cursor-pointer rounded-xl px-3 py-2 text-sm outline-none transition"
                style={{
                  background: p.inputBg,
                  border: `1px solid ${p.cardBorder}`,
                  color: p.textPrimary,
              ...
              >
                ...
                {categoryTree.map(({ parent, children }) => (
                  <optgroup key={parent.id} label={`${parent.icon ?? ""}  ${parent.name}`}>
                    <option value={`parent:${parent.slug}`}>All in {parent.name}</option>
                    {children.map((child) => (
          ...
          >
            <p className="flex items-center gap-2 text-xs" style={{ color: p.textSecondary }}>
              {filtering ? (
                <>
                  <span
            ...
            {hasActiveFilters ? (
              <button
                ...
                className="inline-flex cursor-pointer items-center gap-1 rounded-lg px-2.5 py-1 text-xs font-semibold transition hover:opacity-80"
        ...
        {/* Product grid — while filtering, show the top progress bar + skeleton
            so the user's tap gets instant feedback instead of a "hung" pause */}
        {filtering ? (
          <>
            <TopProgressBar palette={p} />
          ...
          >
            ...
            <button
              onClick={clearFilters}
              className="mt-4 inline-flex cursor-pointer items-center gap-1.5 rounded-lg px-4 py-2 text-xs font-semibold text-white shadow-sm transition hover:opacity-90"
...
/** Indeterminate top progress bar — instant "working on it" signal on every
 *  filter/search change. Sits above the app chrome so it's always visible. */
function TopProgressBar({ palette }: { palette: CataloguePalette }) {
  return (
    <div className="pointer-events-none fixed inset-x-0 top-0 z-[2200] h-[3px] overflow-hidden">
      <div
        className="h-full w-1/3 animate-[cat-progress_0.9s_ease-in-out_infinite]"
        style={{ background: palette.accentGradient, boxShadow: "0 0 10px rgba(8,145,178,0.55)" }}
      />
    </div>
  );
}
...
function Chip({
  ...
  return (
    <button
      onClick={onClick}
      className="cursor-pointer rounded-full px-3 py-1.5 text-xs font-semibold transition hover:brightness-[1.03] active:scale-95"
      style={{

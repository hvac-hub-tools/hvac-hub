...
export default function CompareView({ theme = "light", onBack }: Props) {
  ...
  const fileSlug = items.map((r) => r.product.modelName.replace(/\s+/g, "-")).join("_vs_");
  ...
  async function handleExport() {
    ...
    try {
      if (Capacitor.isNativePlatform()) {
        ...
        const fileName = `HVAC_Comparison_${fileSlug}.xlsx`;
        ...
        setExportMsg({ ok: true, text: "Report ready — share sheet se save kar lo." });
      } else {
        // WEB / PC — styled HTML .xls blob download
        const html = buildExcelHtml();
        const fileName = `HVAC_Comparison_${fileSlug}.xls`;
        ...
        setExportMsg({ ok: true, text: "Excel file download ho rahi hai." });
      }
    } catch (err: any) {
